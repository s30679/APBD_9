...
public class WarehouseService : IWarehouseService
{
    ...
    public async Task<int> DodajProduktProcedureAsync(WarehouseDataE filter, CancellationToken cancellationToken)
    {
        if (filter.Amount<=0)
        {
            throw new ArgumentException("Wartość musi być większa niż 0");
        }

        return await _warehouseRepository.AddProductToWarehouseByProcedureAsync(
            filter.ProductId,
            filter.WarehouseId,
            filter.Amount,
            filter.CreatedAt,
